...
inline NVENCException NVENCException::makeNVENCException(const std::string& errorStr,
                                                         const NVENCSTATUS errorCode,
                                                         const std::string& functionName,
                                                         const std::string& fileName, int lineNo) {
    std::ostringstream errorLog;
    errorLog << functionName << " : " << errorStr << " at " << fileName << ":" << lineNo
             << std::endl;
...
#define NVENC_THROW_ERROR(errorStr, errorCode)                                                     \
    do {                                                                                           \
        throw NVENCException::makeNVENCException(errorStr, errorCode, __FUNCTION__, __FILE__,      \
                                                 __LINE__);                                        \
    } while (0)
...
#define NVENC_API_CALL(nvencAPI)                                                                   \
    do {                                                                                           \
        ...
        if (errorCode != NV_ENC_SUCCESS) {                                                         \
            ...
            throw NVENCException::makeNVENCException(errorLog.str(), errorCode, __FUNCTION__,      \
                                                     __FILE__, __LINE__);                          \
        }                                                                                          \
    } while (0)
...
public:
     ...
     *  GetNextInputFrame() function.
     */
    _NV_ENC_PIC_TYPE EncodeFrame(void* d3d11_texture, std::vector<std::vector<uint8_t>>& vPacket,
                                 NV_ENC_PIC_PARAMS* pPicParams = nullptr);
     ...
     */
    void CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid,
                                    GUID presetGuid);
...
public:
    ...
    /**
     *  @brief This a static function to get the chroma planes height in bytes for YUV planar
     * formats.
     */
...
protected:
    ...
    NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void* pDevice, uint32_t nWidth, uint32_t nHeight,
              NV_ENC_BUFFER_FORMAT eBufferFormat, uint32_t m_nOutputDelay,
    ...
    /**
     *  @brief This function is used to unregister resources which had been previously registered
     * for encoding using RegisterResources() function.
     */
...
private:
    ...
    */
     ...
     */
    _NV_ENC_PIC_TYPE DoEncode(NV_ENC_INPUT_PTR inputBuffer,
     ...
     */
    void DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_INPUT_PTR referenceFrame,
...
private:
    ...
    NV_ENC_REGISTER_RESOURCE direct_register_res_ = {NV_ENC_REGISTER_RESOURCE_VER};
};
