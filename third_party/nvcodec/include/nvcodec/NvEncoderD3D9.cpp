...
NvEncoderD3D9::NvEncoderD3D9(IDirect3DDevice9* pD3D9Device, uint32_t nWidth, uint32_t nHeight,
    ...
    : NvEncoder(NV_ENC_DEVICE_TYPE_DIRECTX, pD3D9Device, nWidth, nHeight, eBufferFormat,
...
void NvEncoderD3D9::AllocateInputBuffers(int32_t numInputBuffers) {
    ...
    for (int count = 0; count < numCount; count++) {
        ...
        for (int i = 0; i < numInputBuffers; i++) {
            ...
            if (m_pDXVAService) {
                res = m_pDXVAService->CreateSurface(
                    GetMaxEncodeWidth(), GetMaxEncodeHeight(), 0, GetD3D9Format(GetPixelFormat()),
                    D3DPOOL_DEFAULT, 0, DXVA2_VideoProcessorRenderTarget, &pD3D9Surface, nullptr);
            }
            else {
                res = m_pD3D9Device->CreateOffscreenPlainSurface(
                    GetMaxEncodeWidth(), GetMaxEncodeHeight(), GetD3D9Format(GetPixelFormat()),
                    D3DPOOL_DEFAULT, &pD3D9Surface, nullptr);
            }
            if (res != S_OK) {
        ...
        RegisterResources(inputFrames, NV_ENC_INPUT_RESOURCE_TYPE_DIRECTX, GetMaxEncodeWidth(),
                          GetMaxEncodeHeight(), 0, GetPixelFormat(), count == 1 ? true : false);
    }
}
