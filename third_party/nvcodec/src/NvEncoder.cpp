...
NvEncoder::NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void* pDevice, uint32_t nWidth,
    ...
    direct_register_res_.version = NV_ENC_REGISTER_RESOURCE_VER;
    direct_register_res_.resourceToRegister = nullptr;
}
...
void NvEncoder::LoadNvEncApi() {
    ...
    if (hModule == NULL) {
        NVENC_THROW_ERROR("NVENC library file is not found. Please ensure NV driver is installed",
    ...
    NvEncodeAPIGetMaxSupportedVersion_Type NvEncodeAPIGetMaxSupportedVersion =
        (NvEncodeAPIGetMaxSupportedVersion_Type)GetProcAddress(hModule,
                                                               "NvEncodeAPIGetMaxSupportedVersion");
#else
    NvEncodeAPIGetMaxSupportedVersion_Type NvEncodeAPIGetMaxSupportedVersion =
        (NvEncodeAPIGetMaxSupportedVersion_Type)dlsym(hModule, "NvEncodeAPIGetMaxSupportedVersion");
#endif
    ...
    if (currentVersion > version) {
        NVENC_THROW_ERROR("Current Driver Version does not support this NvEncodeAPI version, "
    ...
    if (!NvEncodeAPICreateInstance) {
        NVENC_THROW_ERROR("Cannot find NvEncodeAPICreateInstance() entry in NVENC library",
                          NV_ENC_ERR_NO_ENCODE_DEVICE);
    }
...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams,
    ...
    if (pIntializeParams == nullptr || pIntializeParams->encodeConfig == nullptr) {
        NVENC_THROW_ERROR("pInitializeParams and pInitializeParams->encodeConfig can't be NULL",
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncoderParams) {
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_HEVC_GUID) {
        bool yuv10BitFormat = (m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV420_10BIT ||
                               m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT)
                                  ? true
                                  : false;
        if (yuv10BitFormat &&
    ...
    if (pEncoderParams->encodeConfig) {
        ...
        m_nvenc.nvEncGetEncodePresetConfig(m_hEncoder, pEncoderParams->encodeGUID,
                                           NV_ENC_PRESET_DEFAULT_GUID, &presetConfig);
    ...
    m_nEncoderBuffer = m_encodeConfig.frameIntervalP + m_encodeConfig.rcParams.lookaheadDepth +
                       m_nExtraOutputDelay;
...
_NV_ENC_PIC_TYPE NvEncoder::EncodeFrame(void* d3d11_texture,
    ...
    if (direct_register_res_.resourceToRegister &&
        direct_register_res_.resourceToRegister != d3d11_texture) {
        NVENC_API_CALL(
            m_nvenc.nvEncUnregisterResource(m_hEncoder, direct_register_res_.registeredResource));
        direct_register_res_.resourceToRegister = nullptr;
        direct_register_res_.registeredResource = nullptr;
    }
    if (direct_register_res_.resourceToRegister == nullptr) {
        direct_register_res_.resourceToRegister = d3d11_texture;
        NVENC_API_CALL(m_nvenc.nvEncRegisterResource(m_hEncoder, &direct_register_res_));
    }
    int i = m_iToSend % m_nEncoderBuffer;
    NV_ENC_MAP_INPUT_RESOURCE mapInputResource = {NV_ENC_MAP_INPUT_RESOURCE_VER};
    mapInputResource.registeredResource = direct_register_res_.registeredResource;
    NVENC_API_CALL(m_nvenc.nvEncMapInputResource(m_hEncoder, &mapInputResource));

    m_vMappedInputBuffers[i] = mapInputResource.mappedResource;
    auto type = DoEncode(m_vMappedInputBuffers[i], vPacket, pPicParams);

    NVENC_API_CALL(m_nvenc.nvEncUnmapInputResource(m_hEncoder, &mapInputResource));
    if (direct_register_res_.resourceToRegister) {
        NVENC_API_CALL(
            m_nvenc.nvEncUnregisterResource(m_hEncoder, direct_register_res_.registeredResource));
        direct_register_res_.resourceToRegister = nullptr;
...
_NV_ENC_PIC_TYPE NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer,
                                     std::vector<std::vector<uint8_t>>& vPacket,
                                     NV_ENC_PIC_PARAMS* pPicParams) {
    ...
    memset(&encode_stats, 0, sizeof(NV_ENC_STAT));
    encode_stats.outputBitStream = picParams.outputBitstream;
    encode_stats.version = NV_ENC_STAT_VER;
    m_nvenc.nvEncGetEncodeStats(m_hEncoder, &encode_stats);
    return static_cast<_NV_ENC_PIC_TYPE>(encode_stats.picType);
}
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR>& vOutputBuffer,
                                 std::vector<std::vector<uint8_t>>& vPacket, bool bOutputDelay) {
    unsigned i = 0;
...
void NvEncoder::RegisterResources(std::vector<void*> inputframes,
                                  NV_ENC_INPUT_RESOURCE_TYPE eResourceType, int width, int height,
                                  int pitch, NV_ENC_BUFFER_FORMAT bufferFormat,
...
void NvEncoder::GetChromaSubPlaneOffsets(const NV_ENC_BUFFER_FORMAT bufferFormat,
    ...
    case NV_ENC_BUFFER_FORMAT_IYUV:
        chromaOffsets.push_back(pitch * height);
        chromaOffsets.push_back(chromaOffsets[0] + (NvEncoder::GetChromaPitch(bufferFormat, pitch) *
...
uint32_t NvEncoder::GetChromaWidthInBytes(const NV_ENC_BUFFER_FORMAT bufferFormat,
...
void NvEncoder::GetInitializeParams(NV_ENC_INITIALIZE_PARAMS* pInitializeParams) {
    if (!pInitializeParams || !pInitializeParams->encodeConfig) {
        NVENC_THROW_ERROR(
            "Both pInitializeParams and pInitializeParams->encodeConfig can't be NULL",
            NV_ENC_ERR_INVALID_PTR);
    }
...
void NvEncoder::DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer,
    ...
    if (nvStatus == NV_ENC_SUCCESS || nvStatus == NV_ENC_ERR_NEED_MORE_INPUT) {
        ...
        if (vPacket.size() != 1) {
            NVENC_THROW_ERROR("GetEncodedPacket() doesn't return one (and only one) MVData",
                              NV_ENC_ERR_GENERIC);
        }
        mvData = vPacket[0];
    }
