...
Client::Client(const Params& params)
    ...
    , signaling_params_ { params.client_id, params.room_id, params.signaling_addr, params.signaling_port }
    , video_params_ { 
        to_ltrtc(params.codec),
        params.width,
        params.height,
        params.screen_refresh_rate,
        std::bind(&Client::send_message_to_host, this, std::placeholders::_1, std::placeholders::_2 , std::placeholders::_3)
    }
{
}
...
bool Client::init()
{
    ...
    params.on_message = std::bind(&Client::on_signaling_net_message, this, std::placeholders::_1, std::placeholders::_2);
...
void Client::on_signaling_net_message(uint32_t type, std::shared_ptr<google::protobuf::MessageLite> msg)
{
...
void Client::on_join_room_ack(std::shared_ptr<google::protobuf::MessageLite> _msg)
{
    ...
    if (msg->err_code() != ltproto::signaling::JoinRoomAck_ErrCode_Success) {
        LOG(INFO) << "Join room " << signaling_params_.room_id << " with id " << signaling_params_.client_id << " failed";
        return;
    }
...
bool Client::init_ltrtc()
{
    ...
    cfg.on_audio = std::bind(&Client::on_ltrtc_audio_data, this, ph::_1, ph::_2, ph::_3, ph::_4, ph::_5);
...
void Client::on_ltrtc_audio_data(uint32_t bits_per_sample, uint32_t sample_rate, uint32_t number_of_channels, const void* audio_data, uint32_t size)
{
    // audio_module_->submit(bits_per_sample, sample_rate, number_of_channels, reinterpret_cast<const uint8_t*>(audio_data), size);
}
...
void Client::on_ltrtc_connected()
{
    ...
    // input_params_.send_message = std::bind(&ClientImpl::send_message_to_host, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
    // input_params_.host_height = video_params_.height;
    // input_params_.host_width = video_params_.width;
    // input_ = client::Input::create(input_params_);
    // if (input_ == nullptr) {
...
void Client::dispatch_remote_message(uint32_t type, const std::shared_ptr<google::protobuf::MessageLite>& msg)
{
    switch (type) {
...
void Client::on_start_transmission_ack(const std::shared_ptr<google::protobuf::MessageLite>& _msg)
{
    ...
    if (msg->err_code() == ltproto::peer2peer::StartTransmissionAck_ErrCode_Success) {
        ...
        LOG(INFO) << "StartTransmission failed with " << ltproto::peer2peer::StartTransmissionAck_ErrCode_Name(msg->err_code());
        stop_wait();
    }
}
