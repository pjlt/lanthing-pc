...
template <typename T, T M> inline typename std::enable_if<(M > 0), bool>::type AheadOrAt(T a, T b) {
...
// from each other, then the sequence number with the highest value is
// considered to be ahead.
template <typename T, T M = 0> inline bool AheadOf(T a, T b) {
    static_assert(std::is_unsigned<T>::value, "Type must be an unsigned integer.");
...
template <typename T, T M = 0> struct AscendingSeqNumComp {
...
// WARNING! If used to sort sequence numbers of length M then the interval
//          covered by the sequence numbers may not be larger than floor(M/2).
template <typename T, T M = 0> struct DescendingSeqNumComp {
...
template <typename T, T M = 0> class SeqNumUnwrapper {
