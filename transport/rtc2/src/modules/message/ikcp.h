...
#if defined(_WIN64) || defined(WIN64) || defined(__amd64__) || defined(__x86_64) ||                \
    defined(__x86_64__) || defined(_M_IA64) || defined(_M_AMD64)
...
#define IQUEUE_ADD(node, head)                                                                     \
    ((node)->prev = (head), (node)->next = (head)->next, (head)->next->prev = (node),              \
     (head)->next = (node))
...
#define IQUEUE_DEL(entry)                                                                          \
    ((entry)->next->prev = (entry)->prev, (entry)->prev->next = (entry)->next, (entry)->next = 0,  \
     (entry)->prev = 0)
